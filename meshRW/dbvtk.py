...
def getVTKObj(txtElemtype: str) -> tuple:
    """
    Get the vtk class from libvtk from text declaration
    syntax:
        getVTKEObjType(txtElemtype)

    input:
        txtElemtype: element declared using VTK string 
    ...
    numPerElement = -1
    if txtElemtype.upper() in VTKtoElem:
        txtElemtype = VTKtoElem[txtElemtype]
    vtkobj = elementDict[txtElemtype.upper()].get('vtkobj', None)
    numPerElement = getNumberNodes(txtElemtype.upper())
    if not vtkobj:
        Logger.error(f'Element type {txtElemtype} not implemented')
    return vtkobj, numPerElement


def getVTKElemType(txtElemtype: str|int)-> tuple:
    """
    ...
    Args:
        txtElemtype (Union[str, int]): The element type, either as a string (VTK string declaration)
    ...
    if isinstance(txtElemtype, int):
        elementNum = txtElemtype
    elif isinstance(txtElemtype, str):
        elem_type_upper = txtElemtype.upper()
        if elem_type_upper in VTKtoElem:
            elem_type_upper = VTKtoElem[elem_type_upper]
        elementNum = elementDict[elem_type_upper].get('code', None)
        numPerElement = getNumberNodes(elem_type_upper)
    else:
        elem_key = txtElemtype.upper()
...
def getNumberNodesFromNum(elementNum: int) -> int:
    ...
    elementType = getElemTypeFromVTK(elementNum)
    if elementType is None:
        return 0
    return getNumberNodes(elementType)
...
# Keywords MSH
DFLT_HEADER_VERSION: str = '# vtk DataFile Version 2.0'
DFLT_TYPE_ASCII: str = 'ASCII'
DFLT_FILE_VERSION: str = '2.2 0 8'
DFLT_TYPE_MESH: str = 'DATASET UNSTRUCTURED_GRID'
DFLT_NODES: str = 'POINTS'
DFLT_NODES_DATA: str = 'POINT_DATA'
DFLT_DOUBLE: str = 'double'
DFLT_FLOAT: str = 'float'
DFLT_INT: str = 'int'
DFLT_ELEMS: str = 'CELLS'
DFLT_ELEMS_TYPE: str = 'CELL_TYPES'
DFLT_ELEMS_DATA: str = 'CELL_DATA'
DFLT_FIELD: str = 'FIELD'
DFLT_SCALARS: str = 'SCALARS'
DFLT_TABLE: str = 'LOOKUP_TABLE'
DFLT_TABLE_DEFAULT: str = 'default'
